...
public class JHSMySQL {
     ...
     * @return
     */
    public static JHSMySQL getInstance() {
        return SimpleMySQLHolder.INSTANCE;
    }

    private static class SimpleMySQLHolder {

        private static final JHSMySQL INSTANCE = new JHSMySQL();
    }
    ...
    private synchronized void auto_reconnect() {
        ...
        while (retries_left > 0 && !connected) {
            ...
            try {
                ...
                System.err.println("AUTO RECONNECT: " + e.toString());
            } catch (Exception e) {
     ...
     * java.lang.String)
     */
    public JHSMySQLResult Query(String query, String... parms) {
        ...
        try {
            if (query.startsWith("SELECT")) {
                ...
                stmt = mysql_connection.prepareStatement(query);
                int i = 0;
                ...
                // stmt = mysql_connection.prepareStatement(query);
                // stmt.executeUpdate();               
                result = new JHSMySQLResult(stmt.executeQuery());
            } else {
    ...
    public JHSMySQLResult Query(String query) {
        //Make sure connection is alive
        check_connection();
        ...
        try {
            if (query.startsWith("SELECT")) {
                ...
                stmt = mysql_connection.createStatement();
                mysql_result = stmt.executeQuery(query);
                result = new JHSMySQLResult(mysql_result);
            } else {
                //It's an UPDATE, INSERT, or DELETE statement
                //Use the "executeUpdate" function and return a null result
